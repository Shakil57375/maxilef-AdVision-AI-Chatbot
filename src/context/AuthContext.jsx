...
export function AuthProvider({ children }) {
  ...
  const { data: user, isLoading: isProfileLoading } = useGetUserProfileQuery(undefined, {
  ...
  const handleLogin = async (credentials) => {
    try {
      ...
      const { access, refresh, user_profile } = data;
      console.log(access)
      ...
      toast.error("Incorrect email/password, please try again.", {duration : 1000});
      navigate("/login");
    }
  };
  ...
  const handleSignup = async (userData) => {
    try {
      ...
      localStorage.setItem("auth", JSON.stringify(data));
      toast.success("Signup successful! Please verify your email.", {duration : 1000});
      ...
      toast.error("This email is already registered. Please log in or use a different email address.", {duration : 1000});
  ...
  const handleUpdateProfile = async (profileData) => {
    try {
      ...
      localStorage.setItem("auth", JSON.stringify({ ...authData, user_profile: data }));
